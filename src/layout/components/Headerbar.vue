<template>
    <div class="_cover-top">
        <div class="other">
            <span class="iconfont icon-tips-jia"  v-touch:tap="tap"></span>
            ...
            <ul class="tips-menu" :class="tips_isOpen?'tips-open':'tips-close'">
                <li v-for="item in menuArr" :key="item.id" v-link="item._link">
        ...
        <div class="center">
            <!-- {{menu_active.text}} -->
            微信
            <span class="parentheses" v-show='chatCount' v-text="10"></span>
        </div>
    </div>
</template>
<script>
